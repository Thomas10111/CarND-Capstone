...
class WaypointUpdater(object):
    def __init__(self):
        topic = 'chatter'
        self.pub = rospy.Publisher(topic, std_msgs.msg.String, queue_size=100)
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.pose and self.base_lane and self.waypoint_tree:  # why is checking self.waypoint_tree neccessary?
                self.pub.publish("pose and base_lane")
                self.publish_waypoints()
            rate.sleep()

    def get_closest_waypoint_idx(self):
        self.pub.publish("get_closest_waypoint_idx()")
        x = self.pose.pose.position.x
        y = self.pose.pose.position.y
        if self.waypoint_tree is None:
            self.pub.publish("self.waypoint_tree is None.")
    ...
    def publish_waypoints(self):
        self.pub.publish("publish_waypoints")
    ...
    def waypoints_cb(self, waypoints):
        # TODO: Implement
        # self.pub.publish("waypoints_cb")
        self.base_lane = waypoints
        if not self.waypoints_2d:
            self.pub.publish("waypoints_2d")
            self.waypoints_2d = [[waypoint.pose.pose.position.x, waypoint.pose.pose.position.y] for waypoint in waypoints.waypoints]
            # self.pub.publish("waypoints_2d: " + str(self.waypoints_2d[0:10]))
